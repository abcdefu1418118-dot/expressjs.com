...
LIST_END
) | while read -r org repo branch; do
  # Write the README.md to a file named after the repo
  DEST="$DEST_DIR/$repo.md"
  ...
  if [ -z "${branch:-}" ]; then
    ...
    curl -fsS "$NPMURL" | jq -r '.readme' | sed -e 's/[[:space:]]*$//' > "$DEST"
  else
    ...
    if [ "$branch" != "master" ]; then
      # sanitize the branch string for filename use
      safe_branch="${branch//\\//-}"
      DEST="$DEST_DIR/$repo-$safe_branch.md"
    fi
    echo "fetching $org/$repo/$branch from GitHub's raw content domain..."
    if ! curl -fsS "$GHURL" -o "$DEST"; then
      echo "Failed to fetch $GHURL" >&2
    fi
  fi
done
